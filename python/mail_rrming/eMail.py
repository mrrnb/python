...
#############
#读取邮件列表
file_object = open('config/mailto_list.txt')
try:
...
#读取配置文件
file_object = open('config/mail_conf.txt')
try:
    all_the_text = file_object.readlines( )
finally:
    file_object.close( )
...
#读取内容
file_object = open('content/html.txt')
try:
    all_the_text = file_object.readlines( )
finally:
    file_object.close( )
content_list = ''.join(all_the_text).replace("\n","<br />")
#content_list = '<br />'.join(all_the_text)
...
def send_mail(to_list,sub,content,host,user,passwd,postfix,name):
    ...
    attach1 = MIMEText(open('content/html_all.txt','rb').read(),'base64','utf-8')
    attach1["Content-Type"] = 'aplication/octet-stream'
    attach1["Content-Disposition"] = 'attachment; filename="log_all.log"'
    msg.attach(attach1)
